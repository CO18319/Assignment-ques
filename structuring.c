...
{
    struct document solution;
    solution.paragraph_count = 1;
    int i,start = 0,index = 0;;
    for (i = 0; text[i]; i++)
        if (text[i] == '\n')
            ++solution.paragraph_count;
    solution.data = calloc(solution.paragraph_count, sizeof(struct paragraph));
    
    for (i = 0; text[i]; i++)
        if (text[i] == '\n')
        {
           solution.data[index++] = get_paragraph(text, start, i - 1);
            start = i + 1;
        }
    solution.data[index] = get_paragraph(text, start, i - 1);
    return solution;
